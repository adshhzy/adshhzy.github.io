...
/* 
    ...
    */ 
    ...
    DAG.prototype.addDirectedEdge = function(n1, n2) {
        ...
        if (this.lvl[n1] != undefined) if(this.lvl[n1] != this.nLvl) window.alert("wrong usage of DAG");
        this.lvl[n1] = this.nLvl;
        //if (this.lvl[n2] == undefined) this.lvl[n2] = 0;

    };

    DAG.prototype.addNode = function(n1) {
        if (this.adj[n1] == undefined) {
            this.adj[n1] = [];
            this.lvl[n1] = this.nLvl;
        }
...
/* 
    ...
    */
    ...
    var drawStgPoint = function(p, color, rs) { 

        stg.append("circle")
        .attr("id","the_added_ID")
        .attr("cx", p.x + "px")
        .attr("cy", p.y + "px")
        .attr("r", rs+"px")
        .attr("fill", color); 
    ...
    var drawStgLabelPoint = function(p, label, color) { 
    ...
    var drawStgLine = function(a, b, color) { 
        stg.append("line")
        .attr("id","the_added_ID")
        .attr("stroke-width", 2)
        .attr("stroke", color)
    ...
    var drawStgTrunLine = function(a, b, lamnbda, color) { 
        var vx = a.x-b.x;
        var vy = a.y-b.y;

        var vxvy = Math.sqrt(vx*vx+vy*vy);
        vx = vx / vxvy;
        vy = vy / vxvy;

        

        var pa = { x: a.x - lamnbda*vx, y: a.y - lamnbda*vy}; 
        var pb = { x: b.x + lamnbda*vx, y: b.y + lamnbda*vy};

        drawStgLine(pa, pb, color);
    }; 
...
// rmpdag.addDirectedEdge(6, 1); 
// rmpdag.addDirectedEdge(6, 2); 
// rmpdag.addDirectedEdge(7, 3); 
// rmpdag.addDirectedEdge(7, 4);
// rmpdag.addLvlCounter();
// rmpdag.addDirectedEdge(8, 5); 
// rmpdag.addDirectedEdge(8, 6);
// rmpdag.addDirectedEdge(8, 7);  

var drawStg = function() { 


    d3.select("#console").text("Cao!"); 
    var handleClick = function() {
        var pt = d3.mouse(this); 
        var p = { x: pt[0], y: pt[1] }; 
        var last; 
        drawStgPoint(p, "black");

        d3.select("#console").text("Cao!"); 
    };


    stg.on("click", handleClick);
};

var drawStgTree = function(tr, pickNodes) { 


    clearStg();

    // var hasParent = new Array();
    // _.each(Object.keys(tr.adj), function(k_id)
    // { 
    //     if (hasParent[k_id] == undefined) hasParent[k_id] = [];
    //     _.each(tr.adj[k_id], function(t_id) { 
    //         if (hasParent[t_id] == undefined) hasParent[t_id] = [k_id];
    //         else hasParent[t_id].push(k_id); 
    //     });
    ...
    //console.log(tr.adj[0].length);


    var trroot = tr.root();

    var nodeDepth = tr.lvl;


    var nodeEachLvl = new Array();

    _.each(Object.keys(tr.lvl), function(k_id)
    { 
        if (nodeEachLvl[tr.lvl[k_id]] == undefined) nodeEachLvl[tr.lvl[k_id]] = [k_id];
        else nodeEachLvl[tr.lvl[k_id]].push(k_id);
    ...
    var ComputeNodePos = function( nlvl, nNodeInd, nTotal) {
        ...
        var nodeypos = lvldist * (nlvl+1) + hstartdist;

        var p = { x: nodexpos, y: nodeypos}; 

        //drawStgPoint(p, "black");

        //drawStgLabelPoint(p,nLabel,"black");
        return p;
    ...
    _.each(pickNodes, function(pickNode){ 
        if(pickNode != undefined && nodePos[pickNode] != undefined){
            drawStgPoint(nodePos[pickNode], "yellow",20);}
        });

    for(var i = 0; i<nodeEachLvl.length; ++i){
        for(var j = 0; j<nodeEachLvl[i].length; ++j){

            drawStgLabelPoint(nodePos[nodeEachLvl[i][j]] ,nodeEachLvl[i][j],"black");

            //nodePos[nodeEachLvl[i][j]] = drawDots(i,j,nodeEachLvl[i].length);
    ...
    { 
        ...
        _.each(tr.adj[k_id], function(t_id) { 
            if(pickNodes != undefined){
                if(pickNodes.indexOf(Number(t_id)) != -1  && pickNodes.indexOf(Number(k_id)) !=-1 )drawStgTrunLine(pp,nodePos[t_id],15,"red");
                else drawStgTrunLine(pp,nodePos[t_id],15,"black");
            } else drawStgTrunLine(pp,nodePos[t_id],15,"black");

            //if(pickNodes.indexOf(t_id) != -1  )console.log(t_id);

       });

    });

    //console.log(pickNodes);

    //stg.on("click", handleClick);
};

//drawStgTree(rmpdag,5);
...
},{"./dag":1,"./point-location":3,"./util":4,"underscore":6}],3:[function(require,module,exports){
/* 
